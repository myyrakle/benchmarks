...
async fn main() {
    ...
    let queue_arg = &args[1];
    println!("Using queue: {}", queue_arg);

    let queue = queue::new_queue(queue_arg)
        .await
        .expect("Failed to create queue instance");

    queue.ping().await.expect("Failed to ping queue");

    queue.setup().await.expect("Failed to setup queue");

    let csv_text = std::fs::read_to_string("dataset.csv").unwrap();

    let worker_count = 10000; // Couchqueue: 500
        ...
        while let Some(entry) = receiver.recv().await {
            ...
            tokio::spawn(async move {
                ...
                for _ in 0..retry_count {
                    ...
                    match queue.write(&entry.key, &entry.value).await {
                        Ok(_) => {

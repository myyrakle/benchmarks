...
def do_something():
    dataframe = polars.read_csv('measurements.txt', separator=';', new_columns=['name', 'value'])
    # Group by name and calculate min, max, mean, sum, and count
    result = dataframe.groupby('name').agg(

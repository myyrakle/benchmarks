...
async fn main() {
    ...
    if args.len() != 2 {
        ...
        eprintln!(
            "db_type: postgres, mysql, mariadb, mongodb, scylla, cassandra, influxdb_v2, influxdb_v3, timescaledb, couchdb, yugabytedb, cockroachdb, clickhouse, etcd, nats, ydb, tidb, fake"
        );
        std::process::exit(1);
    }
